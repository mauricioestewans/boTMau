...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await browserManager.navigateInstance(params.id, url);

    return NextResponse.json({ message: 'Navigation successful' });
  } catch (error: any) {
    console.error('Error navigating instance:', error);
